...
/// Install the PostgresDatabase CRD into the cluster
pub async fn install_database_crd(client: Client) -> Result<(), CrdError> {
    let crd_yaml = include_str!("../../config/crd/postgres-database.yaml");
    let crd: CustomResourceDefinition = serde_yaml::from_str(crd_yaml)?;

    let crds: Api<CustomResourceDefinition> = Api::all(client);
    let params = PatchParams::apply("integration-test").force();

    tracing::info!("Installing PostgresDatabase CRD...");
    ...
    // Wait for CRD to be established (up to 30 seconds)
    tracing::info!("Waiting for PostgresDatabase CRD to be established...");
    ...
    tracing::info!("PostgresDatabase CRD installed and established");
