...
This project is a Kubernetes operator for PostgreSQL using Patroni/Spilo for HA. Target Kubernetes version is 1.35+.
...
Patroni on Kubernetes patterns:
- Endpoints-based DCS for leader election (no external etcd/consul)
- Service selector labels: spilo-role=master, spilo-role=replica
- Patroni REST API on port 8008 for health checks
- ConfigMap for Patroni bootstrap configuration
- RBAC: ServiceAccount needs endpoints, pods, configmaps access
- Spilo image combines PostgreSQL + Patroni + WAL-G

Operator status conditions for PostgresCluster:
- Ready: Cluster accepting connections
...
- PodDisruptionBudget to protect during maintenance
- Pod anti-affinity for failure domain distribution
- Synchronous vs asynchronous replication trade-offs
- WAL-G for continuous backup to object storage
